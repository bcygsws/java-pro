...
class Master {
     ...
     * 使用多个专门的接口，而不是一个总接口（臃肿的总接口），客户端不应该依赖它不需要的接口
     *
     * 5.迪米特法则 loD或LKP
     * Law of Demeter  (LoD),又叫最少知道原则
     * 一个对象对其它对象应该有最少的了解，又叫最少知道原则（LKP，least knowledge principle），尽量降低类与类之间的耦合
     *
     *
     * 6.里式替换原则 LSP
     * Liskov substitution principle LSP
     * 如果每一个T1类型的对象t1,都有一个T2类型的对象t2;使得T1定义的程序p,在t1换成t2时，程序p没有发生变化。那么，T2就是T1的子
     * 类
     *
     * 6.1 具体来说，一个软件实体适用于父类的话，就一定适用于子类
     * 6.2 在任何公开透明引用父类的地方都能适用子类
     * 6.3 子类对象能够替换父类，而程序逻辑却保持不变
     *
     * 引申含义：子类可以扩展父类的功能，而不能改变父类的原有功能
     * a.子类可以覆盖父类的抽象方法，但不能覆盖父类的非抽象方法
     * b.子类可以增加自己特有的方法
     * c.当子类方法重载父类的父类方法，子类方法的前置条件（输入/入参），要比父类的输入参数宽松
     * d.当子类方法覆盖父类方法时，子类方法的后置条件（输出/返回值），要比父类的输出参数更加严格或者相等
     *
     * 7.合成复用原则-CARP
